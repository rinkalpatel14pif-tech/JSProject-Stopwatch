Stopwatch Project

This is a simple JavaScript Stopwatch Project created using HTML, CSS, and JavaScript. It includes basic stopwatch functionalities such as Start, Stop, Pause, Reset, and Lap timing.

🚀 Features

Start the stopwatch
Pause the stopwatch
Reset the time
Clean UI
Fully responsive (if your design supports it)

📂 Project Structure

Stopwatch.html
All HTML, CSS, and JavaScript code is included inside one file.

▶️ How to Run

Download or clone the repository.
Open Stopwatch.html in any browser.
The stopwatch will start working immediately.

🛠️ Technologies Used

HTML5
CSS3
JavaScript
Bootstrap 5 (for styling buttons, layout, spacing)

🎨 Bootstrap Usage Details

This project also uses Bootstrap classes for styling. Some common classes used:

btn — basic button styling
btn-success, btn-info, btn-danger — colored buttons for Start/Stop/Reset
text-center — center alignment
mt-3, mb-3 — margin top & bottom spacing
container — wrapper for layout
Bootstrap helps to quickly design a clean and responsive layout without writing much CSS.

📄 Original Technologies Used

HTML5
CSS3
JavaScript

📸 Screenshot

![Stopwatch UI](./Screenshot/stopwatch.png)

🤝 Contributing

Pull requests are welcome. For major changes, please open an issue first to discuss what you would like to change.

📄 License

This project is open-source and free to use.
